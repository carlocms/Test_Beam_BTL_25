...
void Plot_FileChi2Value_pol2_pol3()
{
    ...
    while(fin1 >> bar >> side >> val){
	    if(side == "R") map1[bar] = val;
    }

    while(fin2 >> bar >> side >> val){
            if(side == "R") map2[bar] = val;
    }
    ...
    leg->SetHeader(Form("#bf{V_{OV} = 3.00 V | vth2 = 7 DAC | Side R }"), "C");
    ...
    // --- Save
    // -------------------------------
    c->SaveAs("plot_pol2_vs_pol3/Chi2_pol2_vs_pol3_vth7_R.pdf");
}
