...
void Plot_slope_vs_calibFactors(int vth)
{
    ...
    std::string fileSlope = basePath + Form("/TProfile_fit_slope/Limited_FitRange/profileComparison_vth%d.txt", vth);

    std::cout << "fileSlope: " << fileSlope << std::endl;
    std::string fileCalib = basePath + Form("/intercalibration/module_intercalib/RESIDUAL_TOFHIR/RESIDUAL_TOFHIR_calibration_factors_Vov3.00_th%02d_global.txt", vth);
    ...
    if(!finSlope) {
	    ...
	    std::cout << "finSlope ok!" << std::endl;
    }

        if(!finCalib) {

            std::cout << "finCalib ok!" << std::endl;
    }
    ...
    // --- CANVAS L
    // =========================================================
    TCanvas* cL = new TCanvas("cL","Slope vs Calibration L",850,600);

    gL->SetTitle(";Calibration factor (TOFHIR);Slope [ps/a.u.]");
    ...
    cL->SetGrid();
    cL->SaveAs(Form("plots_Limited_FitRange/Slope_vs_Calibration_L_vth%d.pdf", vth));

    // =========================================================
    // --- CANVAS R
    // =========================================================
    TCanvas* cR = new TCanvas("cR","Slope vs Calibration R",850,600);

    gR->SetTitle(";Calibration factor (TOFHIR);Slope [ps/a.u.]");
    ...
    //legR->AddEntry(gR,"Slope vs TOFHIR calibration factor","pl");
...
//);
    ...
    cR->SetGrid();
    cR->SaveAs(Form("plots_Limited_FitRange/Slope_vs_Calibration_R_vth%d.pdf", vth));
