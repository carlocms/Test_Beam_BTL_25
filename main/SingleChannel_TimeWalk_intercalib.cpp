...
//  float cpu[2]{0}, mem[2]={0}, vsz[2]={0}, rss[2]={0};
  ...
  // --- From Loop 2:
  system(Form("mkdir -p %s/Loop2/Optional/ToT/",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop2/DeltaT_Raw/",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop2/Optional/EnergyRatioDUT_LR/",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop2/Optional/DeltaT_vs_ToT/",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop2/Optional/DeltaT_EneBin/",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop2/DeltaT_Mean_Raw/",plotDir.c_str()));
  ...
  system(Form("mkdir -p %s/Loop3/MeanProfile_deltaT_Raw/",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop3/MeanProfileComparison_deltaT_Raw/",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop3/TW_func_comparison/Side_L/",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop3/TW_func_comparison/Side_R/",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop3/Chi2_fitFunc_deltaT_vs_Ene/",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop3/Slope_fitFunc_deltaT_vs_Ene/",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop3/Fit_Function_comparison_differentevth/Side_L/",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop3/Fit_Function_comparison_differentevth/Side_R/",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop3/Slope_fit_Pol1_deltaT_vs_Ene/",plotDir.c_str()));
  ...
  system(Form("mkdir -p %s/Loop4/Sigma_deltaT_EneCorr_vs_bar/",plotDir.c_str()));
  ...
  // -- Loop 6:
  system(Form("mkdir -p %s/Loop6/DeltaT_TW_t1fine_Meant1fineREF_Corr/",plotDir.c_str()));
  ...
  /*
  // --- for geometrical TOF correction
  double TOF_corr[16];
  double angle = 52.0 * TMath::Pi() / 180.0;

  for (int i = 1; i <= 16; i++) {
    TOF_corr[i-1] = (i * 3.12) * (sin(angle) / 0.2997);
    std::cout << TOF_corr[i-1] << std::endl;
  }
  */
  //--- get plot settings
  TCanvas* c;
...
//=====================================================================================
  ...
  for(auto stepLabel : stepLabels)
      ...
      if ( histo )
	{
	  ...
	  c -> Print(Form("%s/Loop1/energy_TW/c_energy_external__Vov%.2f_th%02.0f.png",plotDir.c_str(), Vov, vth));
	  c -> Print(Form("%s/Loop1/energy_TW/c_energy_external__Vov%.2f_th%02.0f.pdf",plotDir.c_str(), Vov, vth));
	  delete c;
	  delete ftemp;
	}
	...
	for(auto LRLabel : LRLabels ) {
	  ...
	  if(!source.compare(TB)){
	    ...
	    f_langaus[index] = new TF1(Form("fit_energy_bar%02d_Vov%.2f_vth_%02.0f",iBar,Vov,vth),langaufun,Vov_LandauMin[Vov],1000.,4);
...
*/
  ...
  for(auto mapIt : trees)
      ...
      for(int entry = 0; entry < nEntries; ++entry)
	{
	  ...
	  std::cout << "anEvent->energyL: " << anEvent->energyL << " | anEvent->energyR: " << anEvent->energyR << std::endl; 
	  ...
	  long long deltaT_Mean_raw = (0.5*(anEvent->timeL + anEvent->timeR) - timeMean_ext);
	  ...
	  if (anEvent->barID == 8 && anEvent->vth == 10)
          {
  ...
  for (auto plot : h1_deltaT_L_ext_ENE) {
	  ...
	  //c -> SetLogy();
	    ...
	    latex = new TLatex(0.40,0.85,Form("#splitline{bar 8, Bin %i}{#splitline{Mean = %f}""{std Dev = %.3f}}", iBin, histo->GetMean(), histo->GetRMS()));
          ...
          c->Print(Form("%s/Loop2/Optional/DeltaT_EneBin/deltaT_L_Energy_bin%02d.png",plotDir.c_str(), iBin));
          c->Print(Form("%s/Loop2/Optional/DeltaT_EneBin/deltaT_L_Energy_bin%02d.pdf",plotDir.c_str(), iBin));
	  delete c;
	  delete latex;
  }
}
      ...
      for(int iBar = 0; iBar < 16; ++iBar)
        {
	      ...
	      if(vth==12) {
		      ...
		      std::cout << "vth: " << vth << " | iBar: " << iBar << " | Mean value: " << fitFunc_deltaT_Raw_R[index2]->GetParameter(1) << " | Sigma Err: " << fitFunc_deltaT_Raw_R[index2]->GetParameter(2) << std::endl;
	      }   
*/
	      ...
	      //---------------------------------------------------------------------
	       ...
	       // -- draw ToT channels side R
            ...
            DrawTH2WithProfile(
                            ...
                            plotDir,"Loop2/Optional/DeltaT_vs_ToT",
                            iBar,Vov,vth,"L");
  ...
  for(auto mapIt : trees)
    {
      ...
      for(int entry = 0; entry < nEntries; ++entry)
        {
          ...
          if( entry%100000 == 0 ) {
            std::cout << ">>> AMPLITUDE-WALK STUDIES -> Loop 3: " << mapIt.first << " reading entry " << entry << " / " << nEntries << " (" << 100.*entry/nEntries << "%)" << "\r" << std::flush;
          }
	  ...
	  {
          //   if (anEvent->energyL > 320. && anEvent->energyL < 400.) {
	  ...
	  {
	     //if (anEvent->energyR > 320. && anEvent->energyR < 400.) {
  ...
  //std::map< std::tuple<int,float,int,std::string>, std::pair<double,double> > Slope_fit_Pol1_deltaT_vs_Ene;
  ...
  for(auto stepLabel : stepLabels)
    {
      ...
      for(int iBar = 0; iBar < 16; ++iBar){
	    ...
	    //Slope_fit_Pol1_deltaT_vs_Ene[std::make_tuple(iBar,Vov,vth,"L")] = std::make_pair(fitFunc_deltaT_EneLCorr_L[index2]->GetParameter(1),0.001);
	    //Slope_fit_Pol1_deltaT_vs_Ene[std::make_tuple(iBar,Vov,vth,"R")] = std::make_pair(fitFunc_deltaT_EneRCorr_R[index2]->GetParameter(1),0.001);
  ...
  PlotGraphsErrors(g_MeanfitFunc_deltaT_Raw_L,g_MeanfitFunc_deltaT_Raw_R,"Mean_DeltaT_Raw_TGraphError","#mu_{#DeltaT} [ps] ",plotDir + "Loop3/MeanfitFunc_deltaT_Raw", 0., 3000.);

/*    
  //----------------------------------------------------------
  //  TGraph: slope TProfile POL1 fit vs bar
  //----------------------------------------------------------

  BuildGraphsErrors(Slope_fit_Pol1_deltaT_vs_Ene,g_Slope_fit_Pol1_deltaT_vs_Ene_L,g_Slope_fit_Pol1_deltaT_vs_Ene_R);
  PlotGraphsErrors(g_Slope_fit_Pol1_deltaT_vs_Ene_L,g_Slope_fit_Pol1_deltaT_vs_Ene_R,"Slope_fit_Pol1_deltaT_vs_Ene","Fit function slope [ps/ADC]",plotDir + "Loop3/Slope_fit_Pol1_deltaT_vs_Ene", -1.5, 0.5);

    SaveProfileComparisonToTxt(
    Slope_fit_Pol1_deltaT_vs_Ene,"/eos/home-c/cgiraldi/MTD/TB_CERN_Sep25/Lab5015Analysis/plots/TProfile_fit_slope/test_pol1");
*/
  ...
  PlotGraphsErrors(g_Slope_fitFunc_deltaT_vs_Ene_L,g_Slope_fitFunc_deltaT_vs_Ene_R,"Slope_fitFunc_deltaT_vs_Ene","Fit function slope [ps/ADC]",plotDir + "Loop3/Slope_fitFunc_deltaT_vs_Ene", -1.5, 0.5);

    SaveProfileComparisonToTxt(
    Slope_fitFunc_deltaT_vs_Ene,"/eos/home-c/cgiraldi/MTD/TB_CERN_Sep25/Lab5015Analysis/plots/TProfile_fit_slope/Limited_FitRange");
...
if(optional_plots){
  ...
  PlotGraphsErrors(g_Chi2_fitFunc_deltaT_vs_Ene_L,g_Chi2_fitFunc_deltaT_vs_Ene_R,"Chi2_fitFunc_deltaT_vs_Ene","#chi^{2}/NDF",plotDir + "Loop3/Chi2_fitFunc_deltaT_vs_Ene", 0., 15.);

    SaveProfileComparisonToTxt(
    Chi2_fitFunc_deltaT_vs_Ene,"/eos/home-c/cgiraldi/MTD/TB_CERN_Sep25/Lab5015Analysis/plots/TProfile_fit_pol2_vs_pol3/chi2/pol3");
  ...
  PlotGraphsErrors(g_mean_profile_deltaT_Raw_L,g_mean_profile_deltaT_Raw_R,"Mean_DeltaT_Raw_TProfile","#mu_{#DeltaT} [ps] ",plotDir + "Loop3/MeanProfile_deltaT_Raw", 0., 3000.);
  ...
  PlotGraphsErrors(g_mean_profileComparison_deltaT_Raw_L,g_mean_profileComparison_deltaT_Raw_R,"Difference_Mean_DeltaT_Raw_TProfile","#mu_{#DeltaT}^{Weighted}-#mu_{#DeltaT}^{Simple} [ps] ",plotDir + "Loop3/MeanProfileComparison_deltaT_Raw", -100., 100.);
  ...
  // --- Side L
  DrawOverlayFits_CorrFunction(fitFunc_deltaT_EneLCorr_L,plotDir,"L");

  // --- Side R
  DrawOverlayFits_CorrFunction(fitFunc_deltaT_EneRCorr_R,plotDir,"R");

  //-----------------------------------------------------------------------------------
  // --- Plot Amplitude walk correction function (T1F) comparison same bar different vth
  //-----------------------------------------------------------------------------------

  DrawOverlayFits_SameBar_DifferentVth(
       fitFunc_deltaT_EneLCorr_L,
       plotDir + "/Loop3/Fit_Function_comparison_differentevth/Side_L",
       "L",
       -500., 500.
       );

  DrawOverlayFits_SameBar_DifferentVth(
       fitFunc_deltaT_EneRCorr_R,
       plotDir + "/Loop3/Fit_Function_comparison_differentevth/Side_R",
       "R",
       -500., 500.
       );
...
if(optional_plots){
  ...
  PlotGraphsErrors(g_Sigma_deltaT_EneCorr_L,g_Sigma_deltaT_EneCorr_R,"Sigma_deltaT_EneCorr","#sigma_{#Delta T} [ps]",plotDir + "Loop4/Sigma_deltaT_EneCorr_vs_bar", 60.,85.);

    
  SaveProfileComparisonToTxt(
    Sigma_deltaT_EneCorr,"/eos/home-c/cgiraldi/MTD/TB_CERN_Sep25/Lab5015Analysis/plots/TProfile_fit_pol2_vs_pol3/Sigma/pol2");
}
      ...
      for(int entry = 0; entry < nEntries; ++entry)
        {
          ...
          if( entry%100000 == 0 ) {
            std::cout << ">>> AMPLITUDE WALK CORRECTION STIDIES: Loop 5: " << mapIt.first << " reading entry " << entry << " / " << nEntries << " (" << 100.*entry/nEntries << "%)" << "\r" << std::flush;
          }
