...
// ----------------------------------------------------
TF1* DrawAndFitHistogram(TH1F* histo,
                         const std::string& canvasName,
                         const std::string& xTitle,
                         const std::string& plotDir,
                         const std::string& subDir,
...
// ----------------------------------------------------
TF1* DrawAndFitProfile(TProfile* profile,
...
//------------------------------------------------------------------------
void computeMeanRMS(TGraphErrors* g, double& mean, double& rms)
{
  int n = g->GetN();
  if(n == 0)
  {
...
int main(int argc, char** argv)
{
  ...
  // --- From Loop 2:
  system(Form("mkdir -p %s/Loop2_ToT/",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop2_deltaT_Raw/",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop2_energyRatioDUT_LR/",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop2_deltaT_vs_ToT/",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop2_EnergyRatio_DUT_REF/Left",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop2_EnergyRatio_DUT_REF/Right",plotDir.c_str()));
  system(Form("mkdir -p %s/Loop2_deltaT_EneBin/",plotDir.c_str()));
  system(Form("mkdir -p %s/MeanfitFunc_deltaT_Raw/",plotDir.c_str()));
      ...
      for(int iBar = 0; iBar < 16; ++iBar)
        {
	      ...
	      fitFunc_energyRatioDUT[index2] = DrawAndFitHistogram(
				               ...
				               plotDir,"Loop2_energyRatioDUT_LR",
				               iBar,Vov,int(vth1));
	      ...
	      mean_fitFunc_deltaT_Raw[std::make_tuple(iBar,Vov,vth1,"L")] = std::make_pair(fitFunc_deltaT_Raw_L[index2]->GetParameter(1), fitFunc_deltaT_Raw_L[index2]->GetParameter(2));
	      mean_fitFunc_deltaT_Raw[std::make_tuple(iBar,Vov,vth1,"R")] = std::make_pair(fitFunc_deltaT_Raw_R[index2]->GetParameter(1), fitFunc_deltaT_Raw_L[index2]->GetParameter(2));
	      }
	      //---------------------------------------------------------------------
	      ...
	      DrawHistogram(h1_energyRatioL_REF[index2],
			     ...
			     "(E_{L}/E_{REF})",
			     plotDir,"Loop2_EnergyRatio_DUT_REF/Left",
	      ...
	      // --- Channel R:
              DrawHistogram(h1_energyRatioR_REF[index2],
                            ...
                            "(E_{R}/E_{REF})",
                            plotDir,"Loop2_EnergyRatio_DUT_REF/Right",
...
PlotGraphs(g_MeanfitFunc_deltaT_Raw_L,g_MeanfitFunc_deltaT_Raw_R,"Mean_DeltaT_Raw_TGraphError","#mu_{#DeltaT} [ps] ",plotDir + "/MeanfitFunc_deltaT_Raw",0., 3000.);
      ...
      for(int iBar = 0; iBar < 16; ++iBar){
	    ...
	    // --- Channel L:
            ...
            fitFunc_deltaT_EneRCorr_R[index2] = DrawAndFitProfile(
                                                ...
                                                CTRMeans_R,CTRSigmas_R,ranges,false,
                                                index1,index2,
                                                "pol3",
            ...
            DrawTH2WithProfile(
                            ...
                            plotDir,"Loop3_deltaT_raw_vs_Energy_scatter",
                            iBar,Vov,vth1,"L");
            ...
            fitFunc_deltaT_L_raw_ERatioREF[index2] = DrawAndFitProfile(
                                                ...
                                                plotDir,"Loop3_DeltaT_raw_vs_EnergyRatioREF",
                                                iBar, Vov, int(vth1),"L",
						true, 0.,3.5);
	    ...
	    // --- Channel L:
            DrawTH2WithProfile(
                            ...
                            plotDir,"Loop3_DeltaT_raw_vs_EnergyRatioREF_scatter",
                            iBar,Vov,vth1,"L",
			    true, 0., 3.5);
	    ...
	    DrawTH2WithProfile(
			    ...
			    plotDir,"Loop3_deltaT_raw_vs_MeanEnergyREF",
			    iBar,Vov,vth1,"L");
            ...
            DrawTH2WithProfile(
                            ...
                            plotDir,"Loop3_EneDUT_vs_MeanEneREF",
                            iBar,Vov,vth1,"L");
  ...
  for(auto mapIt : trees)
    {
          ...
          if( h1_deltaT_L_TWcorr[index2] == NULL )
	  {
	    ...
	    // -- Delta T w/ EneRatioREF corr vs Energy DUT:
	    if(optional_plots){
          ...
          for(int iEnergyBin = 1; iEnergyBin <= nEnergyBins; ++iEnergyBin)
            {
              ...
              // --- Channel L: 
              fitFunc_deltaT_TWcorr_L[index2] = DrawAndFitHistogram(
                                             h1_deltaT_L_TWcorr[index2],
                                             Form("c_h1_deltaT_L_TWcorr_%s", labelLR_energyBin.c_str()),
                                             "#Delta T_{L} w/ TW corr [ps]",
                                             plotDir,"Loop4_deltaT_TW_corr",
                                             iBar,Vov,int(vth1),"L");

              // --- Channel R: 
              fitFunc_deltaT_TWcorr_R[index2] = DrawAndFitHistogram(
            ...
            DrawTH2WithProfile(
                            ...
                            plotDir,"Loop4_deltaT_TWCorr_vs_Energy",
                            iBar,Vov,vth1,"L");
            ...
            fitFunc_deltaT_TWCorrL_t1fineL[index2] = DrawAndFitProfile(
                                                ...
                                                plotDir,"Loop4_deltaT_TWCorr_vs_t1fine",
                                                iBar, Vov, int(vth1),"L");
            ...
            DrawTH2WithProfile(
                            ...
                            "t1fine_{L}^{DUT}","#Delta T_{L} w/ TW corr [ps]",
                            plotDir,"Loop4_deltaT_TWCorr_vs_t1fine_scatter",
                            iBar,Vov,vth1,"L");
            ...
            DrawTH2WithProfile(
              ...
              fitFunc_deltaT_EneRatioREFcorr_L[index2] = DrawAndFitHistogram(
                                             ...
                                             plotDir,"Loop4_deltaT_EneRatioREF_corr",
                                             iBar,Vov,int(vth1),"L");
            ...
            DrawTH2WithProfile(
                            ...
                            plotDir,"Loop4_deltaT_EneRatioREFCorr_vs_energyRatioREF",
                            iBar,Vov,vth1,"L",
			    true, 0., 3.5);
            ...
            fitFunc_deltaT_EneRCorrL_t1fineL[index2] = DrawAndFitProfile(
                                                ...
                                                plotDir,"Loop4_deltaT_EneRatioREFCorr_vs_t1fine",
                                                iBar, Vov, int(vth1),"L");
            ...
            DrawTH2WithProfile(
                            ...
                            "t1fine_{L}^{DUT}","#Delta T_{L} w/ ERatio corr [ps]",
                            plotDir,"Loop4_deltaT_EneRatioREFCorr_vs_t1fine_scatter",
                            iBar,Vov,vth1,"L");
            ...
            //---------------------------------------------------------------------
            // --- TProfile Delta T with TW correction vs Energy Ratio
            //---------------------------------------------------------------------

            // --- Channel L:
            fitFunc_deltaT_L_TWCorr_EneRatioREF[index2] = DrawAndFitProfile(
                                                p1_deltaT_TWCorr_L_vs_energyRatioREF[index2],
                                                CTRMeans_TW_L,CTRSigmas_TW_L,ranges,true,
                                                index1,index2,
                                                "pol3",
                                                Form("c_deltaT_TWCorr_L_vs_energyRatioREF_%s", labelLR_energyBin.c_str()),
                                                "E_{L}^{DUT}/E^{REF}","#Delta T_{L} w/ TW corr [ps]",
                                                plotDir,"Loop4_deltaT_TWCorr_vs_energyRatioREF",
                                                iBar, Vov, int(vth1),"L",
                                                true,0.,3.5);

            // --- Channel R:
            fitFunc_deltaT_R_TWCorr_EneRatioREF[index2] = DrawAndFitProfile(
                                                p1_deltaT_TWCorr_R_vs_energyRatioREF[index2],
                                                CTRMeans_TW_R,CTRSigmas_TW_R,ranges,true,
                                                index1,index2,
                                                "pol3",
                                                Form("c_deltaT_TWCorr_R_vs_energyRatioREF_%s", labelLR_energyBin.c_str()),
                                                "E_{R}^{DUT}/E^{REF}","#Delta T_{R} w/ TW corr [ps]",
                                                plotDir,"Loop4_deltaT_TWCorr_vs_energyRatioREF",
                                                iBar, Vov, int(vth1),"R",
                                                true,0.,3.5);
            ...
            //-------------------------------------------------------------------------------------
            // --- TH2F + TProfile: Delta T with TW correction vs vs Energy Ratio
            //-------------------------------------------------------------------------------------

            // --- Channel L:
            DrawTH2WithProfile(
                            h2_deltaT_TWCorr_L_vs_energyRatioREF[index2],
                            p1_deltaT_TWCorr_L_vs_energyRatioREF[index2],
                            Form("c_deltaT_TWCorr_L_vs_energyRatioREF_scatter_%s", labelLR_energyBin.c_str()),
                            "E_{L}^{DUT}/E^{REF}","#Delta T_{L} w/ TW corr [ps]",
                            plotDir,"Loop4_deltaT_TWCorr_vs_energyRatioREF_scatter",
                            iBar,Vov,vth1,"L");

            // --- Channel R:
            DrawTH2WithProfile(
                            h2_deltaT_TWCorr_R_vs_energyRatioREF[index2],
                            p1_deltaT_TWCorr_R_vs_energyRatioREF[index2],
                            Form("c_deltaT_TWCorr_R_vs_energyRatioREF_scatter_%s", labelLR_energyBin.c_str()),
                            "E_{R}^{DUT}/E^{REF}","#Delta T_{R} w/ TW corr [ps]",
                            plotDir,"Loop4_deltaT_TWCorr_vs_energyRatioREF_scatter",
                            iBar,Vov,vth1,"R");
            ...
            fitFunc_deltaT_EneRatioREF_TW_L[index2] = DrawAndFitProfile(
                                                ...
                                                plotDir,"Loop4_deltaT_EneRatioREFCorr_vs_energy",
                                                iBar, Vov, int(vth1),"L");
            ...
            DrawTH2WithProfile(
                            ...
                            "E_{L}^{DUT}","#Delta T_{L} w/ ERatio corr [ps]",
                            plotDir,"Loop4_deltaT_EneRatioREFCorr_vs_energy_scatter",
                            iBar,Vov,vth1,"L");
            ...
            DrawTH2WithProfile(
                            ...
                            plotDir,"Loop4_deltaT_TWCorr_vs_MeanEneREF",
                            iBar,Vov,vth1,"L");
            ...
            DrawTH2WithProfile(
                            ...
                            plotDir,"Loop4_deltaT_EneRatioREFCorr_vs_MeanEneREF",
                            iBar,Vov,vth1,"L");
            ...
            DrawTH2WithProfile(
                            ...
                            plotDir,"Loop4_t1fineL_vs_t1fineR_scatter",
                            iBar,Vov,vth1,"L_R");
  ...
  for(auto mapIt : trees)
    {
	  ...
	  if( h1_deltaT_TWCorr_EneRatioCorr_L[index2] == NULL )
	  {
	    ...
	    h2_deltaT_TWCorr_EneRatioCorr_vs_energyRatioREF_R[index2] = new TH2F(Form("h2_deltaT_TWCorr_EneRatioCorr_vs_energyRatioREF_R_%s",labelLR_energyBin.c_str()),"",100,0.,5., 2000, -12000., 12000.);
          ...
          if( h1_deltaT_EneRatioREF_TW_Corr_L[index2] == NULL )
          {		  
	  // -- EneRatio + TW Corr  
    ...
    ComputeCTRFromHistoMap(h1_deltaT_TWCorr_EneRatioCorr_L, CTRMeans_TW_EneRatio_L, CTRSigmas_TW_EneRatio_L);
    ComputeCTRFromHistoMap(h1_deltaT_TWCorr_EneRatioCorr_L, CTRMeans_TW_EneRatio_R, CTRSigmas_TW_EneRatio_R);
	...
	for(int iBar = 0; iBar < 16; ++iBar){
		...
		if (optional_plots) {       	
              ...
              // --- Channel L: 
              fitFunc_deltaT_TWCorr_EneRatioCorr_L[index2] = DrawAndFitHistogram(
                                             h1_deltaT_TWCorr_EneRatioCorr_L[index2],
                                             Form("c_h1_deltaT_TWCorr_EneRatioCorr_L_%s", labelLR_energyBin.c_str()),
                                             "#Delta T_{L} w/ TW+ERatio corr [ps]",
                                             plotDir,"Loop5_deltaT_TW_EneRatio_Corr",
                                             iBar,Vov,int(vth1),"L");
              
	      // --- Channel R: 
              fitFunc_deltaT_TWCorr_EneRatioCorr_R[index2] = DrawAndFitHistogram(
                                             h1_deltaT_TWCorr_EneRatioCorr_R[index2],
                                             Form("c_h1_deltaT_TWCorr_EneRatioCorr_R_%s", labelLR_energyBin.c_str()),
                                             "#Delta T_{R} w/ TW+ERatio corr [ps]",
                                             plotDir,"Loop5_deltaT_TW_EneRatio_Corr",
                                             iBar,Vov,int(vth1),"R");
            ...
            //-------------------------------------------------------------------------------------
            // --- TH2F + TProfile: Delta T with TW + Energy Ratio corr vs energy DUT
            //-------------------------------------------------------------------------------------

	    // --- Channel L:  
            DrawTH2WithProfile(
                            h2_deltaT_TWCorr_EneRatioCorr_vs_energy_L[index2],
                            p1_deltaT_TWCorr_EneRatioCorr_vs_energy_L[index2],
                            Form("c_deltaT_TWCorr_EneRatioCorr_vs_energy_L_%s", labelLR_energyBin.c_str()),
                            "Energy_{L}^{DUT} [a.u.]","#Delta T_{L} w/ TW+ERatio corr [ps]",
                            plotDir,"Loop5_deltaT_TW_EneRatio_Corr_vs_Energy",
                            iBar,Vov,vth1,"L");


            // --- Channel R:  
            DrawTH2WithProfile(
                            h2_deltaT_TWCorr_EneRatioCorr_vs_energy_R[index2],
                            p1_deltaT_TWCorr_EneRatioCorr_vs_energy_R[index2],
                            Form("c_deltaT_TWCorr_EneRatioCorr_vs_energy_R_%s", labelLR_energyBin.c_str()),
                            "Energy_{R}^{DUT} [a.u.]","#Delta T_{R} w/ TW+ERatio corr [ps]",
                            plotDir,"Loop5_deltaT_TW_EneRatio_Corr_vs_Energy",
                            iBar,Vov,vth1,"R");
...
//		}//optional_plots
            ...
            // --- Channel L:  
            DrawTH2WithProfile(
                            h2_deltaT_TWCorr_EneRatioCorr_vs_energyRatioREF_L[index2],
                            p1_deltaT_TWCorr_EneRatioCorr_vs_energyRatioREF_L[index2],
                            Form("c_deltaT_TWCorr_EneRatioCorr_vs_energyRatioREF_L_%s", labelLR_energyBin.c_str()),
                            "Energy_{L}^{DUT}/<E^{REF}","#Delta T_{L} w/ TW+ERatio corr [ps]",
                            plotDir,"Loop5_deltaT_TW_EneRatio_Corr_vs_EneRatio_scatter",
                            iBar,Vov,vth1,"L");


            // --- Channel R:  
            DrawTH2WithProfile(
                            h2_deltaT_TWCorr_EneRatioCorr_vs_energyRatioREF_R[index2],
                            p1_deltaT_TWCorr_EneRatioCorr_vs_energyRatioREF_R[index2],
                            Form("c_deltaT_TWCorr_EneRatioCorr_vs_energyRatioREF_R_%s", labelLR_energyBin.c_str()),
                            "Energy_{R}^{DUT}/<E^{REF}","#Delta T_{R} w/ TW+ERatio corr [ps]",
                            plotDir,"Loop5_deltaT_TW_EneRatio_Corr_vs_EneRatio_scatter",
                            iBar,Vov,vth1,"R");
              ...
              //---------------------------------------------------------------------
              // --- TH1F Delta T with Energy Ratio + TW correction
              //---------------------------------------------------------------------

              // --- Channel L: 
              fitFunc_deltaT_EneRatioREF_TW_Corr_L[index2] = DrawAndFitHistogram(
                                             h1_deltaT_EneRatioREF_TW_Corr_L[index2],
                                             Form("c_h1_deltaT_EneRatioREF_TW_Corr_L_%s", labelLR_energyBin.c_str()),
                                             "#Delta T_{L} w/ ERatio+TW corr [ps]",
                                             plotDir,"Loop5_deltaT_EneRatio_TW_Corr",
                                             iBar,Vov,int(vth1),"L");


	      // --- Channel R: 
              fitFunc_deltaT_EneRatioREF_TW_Corr_R[index2] = DrawAndFitHistogram(
                                             h1_deltaT_EneRatioREF_TW_Corr_R[index2],
                                             Form("c_h1_deltaT_EneRatioREF_TW_Corr_R_%s", labelLR_energyBin.c_str()),
                                             "#Delta T_{R} w/ ERatio+TW corr [ps]",
                                             plotDir,"Loop5_deltaT_EneRatio_TW_Corr",
                                             iBar,Vov,int(vth1),"R");


            //-------------------------------------------------------------------------------------
            // --- TH2F + TProfile: Delta T with Energy Ratio + TW corr vs energy DUT
            //-------------------------------------------------------------------------------------

            // --- Channel L:  
            DrawTH2WithProfile(
                            h2_deltaT_EneRatioREF_TW_Corr_vs_energy_L[index2],
                            p1_deltaT_EneRatioREF_TW_Corr_vs_energy_L[index2],
                            Form("c_deltaT_EneRatioREF_TW_Corr_vs_energy_L_%s", labelLR_energyBin.c_str()),
                            "Energy_{L}^{DUT} [a.u.]","#Delta T_{L} w/ ERatio+TW corr [ps]",
                            plotDir,"Loop5_deltaT_EneRatio_TW_Corr_vs_Energy_scatter",
                            iBar,Vov,vth1,"L");


            // --- Channel R:  
            DrawTH2WithProfile(
                            h2_deltaT_EneRatioREF_TW_Corr_vs_energy_R[index2],
                            p1_deltaT_EneRatioREF_TW_Corr_vs_energy_R[index2],
                            Form("c_deltaT_EneRatioREF_TW_Corr_vs_energy_R_%s", labelLR_energyBin.c_str()),
                            "Energy_{R}^{DUT} [a.u.]","#Delta T_{R} w/ ERatio+TW corr [ps]",
                            plotDir,"Loop5_deltaT_EneRatio_TW_Corr_vs_Energy_scatter",
                            iBar,Vov,vth1,"R");


            //-------------------------------------------------------------------------------------
            // --- TH2F + TProfile: Delta T with Energy Ratio + TW corr vs Energy Ratio
            //-------------------------------------------------------------------------------------

            // --- Channel L:  
            DrawTH2WithProfile(
                            h2_deltaT_EneRatioREF_TW_Corr_vs_energyRatioREF_L[index2],
                            p1_deltaT_EneRatioREF_TW_Corr_vs_energyRatioREF_L[index2],
                            Form("c_deltaT_EneRatioREF_TW_Corr_vs_energyRatioREF_L_%s", labelLR_energyBin.c_str()),
                            "E_{L}^{DUT}/<E^{REF}>","#Delta T_{L} w/ ERatio+TW corr [ps]",
                            plotDir,"Loop5_deltaT_EneRatio_TW_Corr_vs_EneRatioREF_scatter",
                            iBar,Vov,vth1,"L");


            // --- Channel R:  
            DrawTH2WithProfile(
                            h2_deltaT_EneRatioREF_TW_Corr_vs_energyRatioREF_R[index2],
                            p1_deltaT_EneRatioREF_TW_Corr_vs_energyRatioREF_R[index2],
                            Form("c_deltaT_EneRatioREF_TW_Corr_vs_energyRatioREF_R_%s", labelLR_energyBin.c_str()),
                            "E_{R}^{DUT}/<E^{REF}>","#Delta T_{R} w/ ERatio+TW corr [ps]",
                            plotDir,"Loop5_deltaT_EneRatio_TW_Corr_vs_EneRatioREF_scatter",
                            iBar,Vov,vth1,"R");
              ...
              // --- Channel L:
              fitFunc_deltaT_TW_t1fineCorr_L[index2] = DrawAndFitHistogram(
                                             ...
                                             plotDir,"Loop5_deltaT_TW_t1fine_Corr",
                                             iBar,Vov,int(vth1),"L");


              // --- Channel R:
              fitFunc_deltaT_TW_t1fineCorr_R[index2] = DrawAndFitHistogram(
            ...
            DrawTH2WithProfile(
                            ...
                            plotDir,"Loop5_deltaT_EneRatioREF_t1fine_Corr_vs_Meant1fineREF_scatter",
                            iBar,Vov,vth1,"L");
            ...
            fitFunc_deltaT_EneRatio_t1fineCorr_L[index2] = DrawAndFitProfile(
                                                p1_deltaT_EneRatioREF_t1fine_L_Corr_vs_Meant1fineREF[index2],
                                                CTRMeans_TW_EneRatio_L,CTRSigmas_TW_EneRatio_L,ranges,false,
                                                index1,index2,
                                                "pol3",
                                                Form("c_deltaT_EneRatioREF_t1fine_L_Corr_vs_Meant1fineREF_%s", labelLR_energyBin.c_str()),
                                                "<t1fine^{REF}>","#Delta T_{L} w/ ERatio+t1fine^{DUT} corr [ps]",
                                                plotDir,"Loop5_deltaT_EneRatioREF_t1fine_Corr_vs_Meant1fineREF",
                                                iBar, Vov, int(vth1),"L");

            // --- Channel R:
            fitFunc_deltaT_EneRatio_t1fineCorr_R[index2] = DrawAndFitProfile(
                                                p1_deltaT_EneRatioREF_t1fine_R_Corr_vs_Meant1fineREF[index2],
                                                CTRMeans_TW_EneRatio_R,CTRSigmas_TW_EneRatio_R,ranges,false,
                                                index1,index2,
                                                "pol3",
                                                Form("c_deltaT_EneRatioREF_t1fine_R_Corr_vs_Meant1fineREF_%s", labelLR_energyBin.c_str()),
                                                "<t1fine^{REF}>","#Delta T_{R} w/ ERatio+t1fine^{DUT} corr [ps]",
            ...
            DrawTH2WithProfile(
                            ...
                            plotDir,"Loop5_deltaT_TW_t1fine_Corr_vs_Meant1fineREF",
                            iBar,Vov,vth1,"L");
  ...
  for(auto mapIt : trees)
    ...
    for(auto stepLabel : stepLabels)
    {
            ...
            for(int iBar = 0; iBar < 16; ++iBar){
              ...
              // --- Channel L:
              fitFunc_deltaT_EneRatioREF_t1fine_Meant1fineREF_Corr_L[index2] = DrawAndFitHistogram(
              ...
              // --- Channel R:
              fitFunc_deltaT_EneRatioREF_t1fine_Meant1fineREF_Corr_R[index2] = DrawAndFitHistogram(
...
*/
              ...
              // --- Channel L:
              fitFunc_deltaT_TW_t1fine_Meant1fineREF_Corr_L[index2] = DrawAndFitHistogram(
                                             ...
                                             plotDir,"Loop6_deltaT_TW_t1fine_Meant1fineREF_Corr",
                                             iBar,Vov,int(vth1),"L");

              // --- Channel R:
              fitFunc_deltaT_TW_t1fine_Meant1fineREF_Corr_R[index2] = DrawAndFitHistogram(
