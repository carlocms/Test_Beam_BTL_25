...
else
{
  ...
  for(auto stepLabel : stepLabels)
      ...
      if ( histo )
	{
	  ...
	  c -> Print(Form("%s/Energy_histograms/Loop1_energy_RAW/c_energy_external__Vov%.2f_th%02.0f.png",plotDir.c_str(), Vov, vth1));
	  c -> Print(Form("%s/Energy_histograms/Loop1_energy_RAW/c_energy_external__Vov%.2f_th%02.0f.pdf",plotDir.c_str(), Vov, vth1));
	  delete c;
	  delete ftemp;
	}
