...
#include "interface/Ene_Intercalib_Utils.h"
...
int main(int argc, char** argv)
{
  ...
  std::string LO_mode = "global";
  std::string outputDirTxt = opts.GetOpt<std::string>("Intercalib.OutputData");
  ...
  system(Form("mkdir -p %s/Energy_histograms/Loop1_energy_RAW/",plotDir.c_str()));
 
  // --- From Loop 2:
  system(Form("mkdir -p %s/Energy_histograms/Loop2_energyLO_calib/Left/",plotDir.c_str()));
  system(Form("mkdir -p %s/Energy_histograms/Loop2_energyLO_calib/Right/",plotDir.c_str()));
  system(Form("mkdir -p %s/Scatter_plots/Loop2_scatter_energyLO_calib_L_vs_R/",plotDir.c_str())); 

  // --- Chi2 and MPV check historgrams:
  system(Form("mkdir -p %s/MPV_values/Chi2_histograms/", plotDir.c_str()));
  system(Form("mkdir -p %s/MPV_values/MPV_histograms/", plotDir.c_str()));

  // --- From calibration factors plots:
  system(Form("mkdir -p %s/Calibration_factors/LO_calibration_factors/",plotDir.c_str()));
  system(Form("mkdir -p %s/Calibration_factors/RESIDUAL_TOFHIR_calibration_factors/",plotDir.c_str())); 
  system(Form("mkdir -p %s/Calibration_factors/TOFHIR_calibration_factors/",plotDir.c_str()));
  system(Form("mkdir -p %s/Calibration_factors/COMBINED_calibration_factors/",plotDir.c_str()));

  // --- From Loop 3:
  system(Form("mkdir -p %s/Energy_histograms/Loop3_energy_RESIDUAL_TOFHIR_calib/Left/",plotDir.c_str()));
  system(Form("mkdir -p %s/Energy_histograms/Loop3_energy_RESIDUAL_TOFHIR_calib/Right/",plotDir.c_str()));
  system(Form("mkdir -p %s/Energy_histograms/Loop3_energy_COMBINED_calib/Left/",plotDir.c_str()));
  system(Form("mkdir -p %s/Energy_histograms/Loop3_energy_COMBINED_calib/Right/",plotDir.c_str()));
  system(Form("mkdir -p %s/Energy_histograms/Loop3_energy_TOFHIR_calib/Left/",plotDir.c_str()));
  system(Form("mkdir -p %s/Energy_histograms/Loop3_energy_TOFHIR_calib/Right/",plotDir.c_str()));
  system(Form("mkdir -p %s/Scatter_plots/Loop3_scatter_energy_RESIDUAL_TOFHIR_calib_L_vs_R/",plotDir.c_str()));
  system(Form("mkdir -p %s/Scatter_plots/Loop3_scatter_energy_COMBINED_calib_L_vs_R/",plotDir.c_str()));
  system(Form("mkdir -p %s/Scatter_plots/Loop3_scatter_energy_TOFHIR_calib_L_vs_R/",plotDir.c_str()));

  // --- From MPV calibration:
  system(Form("mkdir -p %s/MPV_values/MPV_RAW/",plotDir.c_str()));
  system(Form("mkdir -p %s/MPV_values/MPV_LO_calib/",plotDir.c_str()));
  system(Form("mkdir -p %s/MPV_values/MPV_COMBINED_calib/",plotDir.c_str()));
  system(Form("mkdir -p %s/MPV_values/MPV_RESIDUAL_TOFHIR_calib/",plotDir.c_str()));
  system(Form("mkdir -p %s/MPV_values/MPV_TOFHIR_calib/",plotDir.c_str()));
  system(Form("mkdir -p %s/MPV_values/MPV_summary/", plotDir.c_str()));

  // --- From MPV Ratio plots:
  system(Form("mkdir -p %s/MPV_Ratio/MPV_ratio_RAW/", plotDir.c_str()));
  system(Form("mkdir -p %s/MPV_Ratio/MPV_Ratio_LO/", plotDir.c_str()));
  system(Form("mkdir -p %s/MPV_Ratio/MPV_Ratio_RESIDUAL_TOFHIR/", plotDir.c_str()));
  system(Form("mkdir -p %s/MPV_Ratio/MPV_Ratio_TOFHIR/", plotDir.c_str()));
  system(Form("mkdir -p %s/MPV_Ratio/MPV_Ratio_COMBINED/", plotDir.c_str()));
...
else
{
  ...
  //--- define histograms
  std::string outFileName = opts.GetOpt<std::string>("Output.outFileName_Intercalib");
  ...
  // MPV Ratio:
  std::map<std::pair<float,int>, TGraph*> gMPV_Ratio_RAW;
  std::map<std::pair<float,int>, TGraph*> gMPV_LO_Ratio;
  std::map<std::pair<float,int>, TGraph*> gMPV_RESIDUAL_TOFHIR_Ratio;
  std::map<std::pair<float,int>, TGraph*> gMPV_TOFHIR_Ratio;
  std::map<std::pair<float,int>, TGraph*> gMPV_COMBINED_Ratio;
  ...
  for(auto stepLabel : stepLabels)
      ...
      if ( histo )
	{
	  ...
	  c -> Print(Form("%s/Energy_histograms/Loop1_energy_RAW/c_energy_external__Vov%.2f_th%02.0f.png",plotDir.c_str(), Vov, vth));
	  c -> Print(Form("%s/Energy_histograms/Loop1_energy_RAW/c_energy_external__Vov%.2f_th%02.0f.pdf",plotDir.c_str(), Vov, vth));
	  delete c;
	  delete ftemp;
	}
	...
	for(auto LRLabel : LRLabels ) {
	  ...
	  latex = new TLatex(0.40,0.85,Form("#splitline{bar %02d%s RAW}{V_{OV} = %.2f V, th. = %d DAC}",iBar,LRLabel.c_str(),Vov,int(vth)));
	  if (LRLabel == "L-R") {
	  ...
	  if(!source.compare(TB)){
	    ...
	    if (f_landau[index]->GetNDF() > 0 && LRLabel !="L-R")
	    {
		    ...
		    MPV_RAW_map[std::make_tuple(iBar,Vov,vth, LRLabel)] = mpv_raw;
              }
	  ...
	  histo->Write();
	  c -> Update();
	  c -> Print(Form("%s/Energy_histograms/Loop1_energy_RAW/c_energy__%s.png",plotDir.c_str(),label.c_str()));
	  c -> Print(Form("%s/Energy_histograms/Loop1_energy_RAW/c_energy__%s.pdf",plotDir.c_str(),label.c_str()));
	  delete c;
	  delete latex;
...
std::cout << std::endl;
std::cout << "[DRAW LOOP1 ---> ended" << std::endl;
std::cout << std::endl;
      ...
      for(int iBar = 0; iBar < 16; ++iBar)
        {
	      ...
	      {
		      MPV_LO_map[std::make_tuple(iBar,Vov,vth,"L")] = fitRes_LOcalib_L.mpv;
		      MPV_values_map[std::make_pair(Vov,int(vth))].push_back(fitRes_LOcalib_L.mpv);
		      MPV_values_map_side[std::make_tuple(Vov,vth,"L")].push_back(fitRes_LOcalib_L.mpv);
		      Chi2_values_LO_map[std::make_tuple(iBar,Vov,vth,"L")] = fitRes_LOcalib_L.chi2_ndf;
	      }

	      latex = new TLatex(0.20,0.85,Form("#splitline{bar %02d L - with LO calibration}{V_{OV} = %.2f V, th. = %d DAC}",iBar,Vov,int(vth)));
	      latex -> SetNDC();
	      latex -> SetTextFont(42);
	      latex -> SetTextSize(0.04);
	      ...
	      c -> Print(Form("%s/Energy_histograms/Loop2_energyLO_calib/Left/c_energyLO_calib_%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
	      c -> Print(Form("%s/Energy_histograms/Loop2_energyLO_calib/Left/c_energyLO_calib_%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
	      delete latex;
	      delete c;
	      ...
	      // --- Landau Fit
              ...
              {
                      MPV_LO_map[std::make_tuple(iBar,Vov,vth,"R")] = fitRes_LOcalib_R.mpv;
                      MPV_values_map[std::make_pair(Vov,int(vth))].push_back(fitRes_LOcalib_R.mpv);
                      MPV_values_map_side[std::make_tuple(Vov,vth,"R")].push_back(fitRes_LOcalib_R.mpv);
                      Chi2_values_LO_map[std::make_tuple(iBar,Vov,vth,"R")] = fitRes_LOcalib_R.chi2_ndf;
              }


              latex = new TLatex(0.20,0.85,Form("#splitline{bar %02d R - with LO calibration}{V_{OV} = %.2f V, th. = %d DAC}",iBar,Vov,int(vth)));
              latex -> SetNDC();
              latex -> SetTextFont(42);
              latex -> SetTextSize(0.04);
              latex -> SetTextColor(kBlack);
              ...
              c -> Print(Form("%s/Energy_histograms/Loop2_energyLO_calib/Right/c_energyLO_calib_%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
              c -> Print(Form("%s/Energy_histograms/Loop2_energyLO_calib/Right/c_energyLO_calib_%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
              delete latex;
              delete c;
	      ...
	      if(!h2_energy_LOcalib_L_vs_R[index2]) continue;
	       ...
	       c -> Print(Form("%s/Scatter_plots/Loop2_scatter_energyLO_calib_L_vs_R/c_energyLO_calib_L_vs_R_%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
	       c -> Print(Form("%s/Scatter_plots/Loop2_scatter_energyLO_calib_L_vs_R/c_energyLO_calib_L_vs_R_%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
               delete c;
               delete latex;
...
std::cout << std::endl;
std::cout << "[Loop 2] ---> ended" << std::endl;
std::cout << std::endl;
...
std::map< std::pair<float,int>, double > MPV_mean_map;                       // map [vov, vth] -> Average MPV vallues for all the channels 
std::map< std::tuple<float,int,std::string>, double > MPV_mean_map_side;     // map [vov,vth,side] -> Average MPV vallues for the channels in the same side

std::map< std::tuple<int,float,int,std::string>, double > RESIDUAL_TOFHIR_factors; // map [bar, vov, vth, side] -> RESIDUAL_TOFHIR_factor  
std::map< std::tuple<int,float,int,std::string>, double > TOFHIR_factors;          // map [bar, vov, vth, side] -> RESIDUAL_TOFHIR_factor / LO_factor
std::map< std::tuple<int,float,int,std::string>, double > COMBINED_factors;        // map [bar, vov, vth, side] -> COMBINED_factor
...
if (LO_mode == "global") // averaging over all the channels of the module (both te side L and R)
{
...
//
// Residual term in the energy intercalibraton once filter out the LO contribution
//----------------------------------------------------------------------------------
...
SaveCalibrationFactorsToFile(RESIDUAL_TOFHIR_factors,outputDirTxt,"RESIDUAL_TOFHIR",LO_mode);

SaveCalibrationFactorsToFile(COMBINED_factors,outputDirTxt,"COMBINED",LO_mode);
...
g_LO_L->SetTitle(Form(";Bar index; LO Calibration"));
...
c_LO->Print(Form("%s/Calibration_factors/LO_calibration_factors/LO_factors_vs_bar.pdf", plotDir.c_str()));
c_LO->Print(Form("%s/Calibration_factors/LO_calibration_factors/LO_factors_vs_bar.png", plotDir.c_str()));
delete leg;
delete c_LO;
...
PlotGraphs(g_RESIDUAL_TOFHIR_L,g_RESIDUAL_TOFHIR_R,"RESIDUAL_TOFHIR","Residual TOFHIR calibration",plotDir + "/Calibration_factors/RESIDUAL_TOFHIR_calibration_factors",0.5, 1.5);
...
PlotGraphs(g_COMBINED_calib_factors_L,g_COMBINED_calib_factors_R,"COMBINED_factors","Combined calibration",plotDir + "/Calibration_factors/COMBINED_calibration_factors",0.5, 1.5);

//---------------------------
// Plot of the TOFHIR factors
//---------------------------
 
BuildGraphs(TOFHIR_factors,g_TOFHIR_L,g_TOFHIR_R);
PlotGraphs(g_TOFHIR_L,g_TOFHIR_R,"TOFHIR_factors","TOFHIR calibration",plotDir + "/Calibration_factors/TOFHIR_calibration_factors",0.75, 1.35);


std::cout << std::endl;
std::cout << "[TGRAPH CALIBRATION FACTORS] ---> done!" << std::endl;
std::cout << std::endl;
  ...
  for(auto mapIt : trees)
    {
	  ...
	  */
	  ...
	  auto key_RESIDUAL_TOFHIR_L = std::make_tuple(anEvent->barID, anEvent->Vov, anEvent->vth, "L");
          auto key_RESIDUAL_TOFHIR_R = std::make_tuple(anEvent->barID, anEvent->Vov, anEvent->vth, "R");
	  ...
	  // --- Key to access RESIDUAL_TOFHIR factors
	  auto key_TOFHIR_L = std::make_tuple(anEvent->barID, anEvent->Vov, anEvent->vth, "L");
          auto key_TOFHIR_R = std::make_tuple(anEvent->barID, anEvent->Vov, anEvent->vth, "R");
      ...
      for(int iBar = 0; iBar < 16; ++iBar)
        {
          ...
          for(int iEnergyBin = 1; iEnergyBin <= nEnergyBins; ++iEnergyBin)
            {
              ...
              {       
                      MPV_COMBINED_map[std::make_tuple(iBar,Vov,vth,"L")] = fitRes_COMBINEDcalib_L.mpv;
                      Chi2_values_COMBINED_map[std::make_tuple(iBar,Vov,vth,"L")] = fitRes_COMBINEDcalib_L.chi2_ndf;
              }

              latex = new TLatex(0.20,0.85,Form("#splitline{bar %02d L - COMBINED calibration}{V_{OV} = %.2f V, th. = %d DAC}",iBar,Vov,int(vth)));
              latex -> SetNDC();
              latex -> SetTextFont(42);
              latex -> SetTextSize(0.04);
              latex -> SetTextColor(kBlack);
              ...
              c -> Print(Form("%s/Energy_histograms/Loop3_energy_COMBINED_calib/Left/c_energy_COMBINED_calib_%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
              c -> Print(Form("%s/Energy_histograms/Loop3_energy_COMBINED_calib/Left/c_energy_COMBINED_calib_%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
              delete latex;
              delete c;
	      ...
	      // -- energy histogram side R: COMBINED calibration
              ...
              {
                      MPV_COMBINED_map[std::make_tuple(iBar,Vov,vth,"R")] = fitRes_COMBINEDcalib_R.mpv;
                      Chi2_values_COMBINED_map[std::make_tuple(iBar,Vov,vth,"R")] = fitRes_COMBINEDcalib_R.chi2_ndf;
              }

              latex = new TLatex(0.20,0.85,Form("#splitline{bar %02d R - COMBINED calibration}{V_{OV} = %.2f V, th. = %d DAC}",iBar,Vov,int(vth)));
              latex -> SetNDC();
              latex -> SetTextFont(42);
              latex -> SetTextSize(0.04);
              latex -> SetTextColor(kBlack);
              ...
              c -> Print(Form("%s/Energy_histograms/Loop3_energy_COMBINED_calib/Right/c_energy_COMBINED_calib_%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
              c -> Print(Form("%s/Energy_histograms/Loop3_energy_COMBINED_calib/Right/c_energy_COMBINED_calib_%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
              delete latex;
              delete c;
              ...
              latex = new TLatex(0.40,0.85,Form("#splitline{bar %02d - COMBINED calib}{V_{OV} = %.2f V, th. = %d DAC}",iBar,Vov,int(vth)));
              latex -> SetNDC();
              latex -> SetTextFont(42);
              latex -> SetTextSize(0.04);
              latex -> SetTextColor(kBlack);
              ...
              c -> Print(Form("%s/Scatter_plots/Loop3_scatter_energy_COMBINED_calib_L_vs_R/c_energy_COMBINED_calib_L_vs_R_%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
              c -> Print(Form("%s/Scatter_plots/Loop3_scatter_energy_COMBINED_calib_L_vs_R/c_energy_COMBINED_calib_L_vs_R_%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
              delete c;
              delete latex;
	      ...
	      // -- energy histogram side L: RESIDUAL TOFHIR calibration
              ...
              {
                      MPV_RESIDUAL_TOFHIR_map[std::make_tuple(iBar,Vov,vth,"L")] = fitRes_RESIDUAL_calib_L.mpv;
                      Chi2_values_RESIDUAL_TOFHIR_map[std::make_tuple(iBar,Vov,vth,"L")] = fitRes_RESIDUAL_calib_L.chi2_ndf;
              }

              latex = new TLatex(0.20,0.85,Form("#splitline{bar %02d L - RESIDUAL TOFHIR calibration}{V_{OV} = %.2f V, th. = %d DAC}",iBar,Vov,int(vth)));
              latex -> SetNDC();
              latex -> SetTextFont(42);
              latex -> SetTextSize(0.04);
              latex -> SetTextColor(kBlack);
              ...
              c -> Print(Form("%s/Energy_histograms/Loop3_energy_RESIDUAL_TOFHIR_calib/Left/c_energy_RESIDUAL_TOFHIR_calib_%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
              c -> Print(Form("%s/Energy_histograms/Loop3_energy_RESIDUAL_TOFHIR_calib/Left/c_energy_RESIDUAL_TOFHIR_calib_%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
	      ...
	      // --- Landau Fit
              ...
              {
                      MPV_RESIDUAL_TOFHIR_map[std::make_tuple(iBar,Vov,vth,"R")] = fitRes_RESIDUAL_calib_R.mpv;
                      Chi2_values_RESIDUAL_TOFHIR_map[std::make_tuple(iBar,Vov,vth,"R")] = fitRes_RESIDUAL_calib_R.chi2_ndf;
              }

              
              latex = new TLatex(0.20,0.85,Form("#splitline{bar %02d R - RESIDUAL TOFHIR calibration}{V_{OV} = %.2f V, th. = %d DAC}",iBar,Vov,int(vth)));
              latex -> SetNDC();
              latex -> SetTextFont(42);
              latex -> SetTextSize(0.04);
              latex -> SetTextColor(kBlack);
              ...
              c -> Print(Form("%s/Energy_histograms/Loop3_energy_RESIDUAL_TOFHIR_calib/Right/c_energy_RESIDUAL_TOFHIR_calib_%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
              c -> Print(Form("%s/Energy_histograms/Loop3_energy_RESIDUAL_TOFHIR_calib/Right/c_energy_RESIDUAL_TOFHIR_calib_%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
              delete latex;
              delete c;
              ...
              latex = new TLatex(0.40,0.85,Form("#splitline{bar %02d - RESIDUAL TOFHIR calib}{V_{OV} = %.2f V, th. = %d DAC}",iBar,Vov,int(vth)));
              latex -> SetNDC();
              latex -> SetTextFont(42);
              latex -> SetTextSize(0.04);
              latex -> SetTextColor(kBlack);
              ...
              c -> Print(Form("%s/Scatter_plots/Loop3_scatter_energy_RESIDUAL_TOFHIR_calib_L_vs_R/c_energy_RESIDUAL_TOFHIR_calib_L_vs_R_%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
              c -> Print(Form("%s/Scatter_plots/Loop3_scatter_energy_RESIDUAL_TOFHIR_calib_L_vs_R/c_energy_RESIDUAL_TOFHIR_calib_L_vs_R_%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
              delete c;
              delete latex;
             ...
             // -- energy histrogram side L: TOFHIR calibration
              ...
              {
                      MPV_TOFHIR_map[std::make_tuple(iBar,Vov,vth,"L")] = fitRes_TOFHIR_calib_L.mpv;
                      Chi2_values_TOFHIR_map[std::make_tuple(iBar,Vov,vth,"L")] = fitRes_TOFHIR_calib_L.chi2_ndf;
              }

              latex = new TLatex(0.20,0.85,Form("#splitline{bar %02d L - TOFHIR calibration}{V_{OV} = %.2f V, th. = %d DAC}",iBar,Vov,int(vth)));
              latex -> SetNDC();
              latex -> SetTextFont(42);
              latex -> SetTextSize(0.04);
              latex -> SetTextColor(kBlack);
              ...
              c -> Print(Form("%s/Energy_histograms/Loop3_energy_TOFHIR_calib/Left/c_energyTOFHIR_calib_%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
              c -> Print(Form("%s/Energy_histograms/Loop3_energy_TOFHIR_calib/Left/c_energyTOFHIR_calib_%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
              delete latex;
              delete c;
             ...
             histo -> Write();
              ...
              {
                      MPV_TOFHIR_map[std::make_tuple(iBar,Vov,vth,"R")] = fitRes_TOFHIR_calib_R.mpv;
                      Chi2_values_TOFHIR_map[std::make_tuple(iBar,Vov,vth,"R")] = fitRes_TOFHIR_calib_R.chi2_ndf;
              }
	     
	     latex = new TLatex(0.20,0.85,Form("#splitline{bar %02d R - TOFHIR calibration}{V_{OV} = %.2f V, th. = %d DAC}",iBar,Vov,int(vth)));
             latex -> SetNDC();
             latex -> SetTextFont(42);
             latex -> SetTextSize(0.04);
             latex -> SetTextColor(kBlack);
             ...
             c -> Print(Form("%s/Energy_histograms/Loop3_energy_TOFHIR_calib/Right/c_energyTOFHIR_calib_%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
             c -> Print(Form("%s/Energy_histograms/Loop3_energy_TOFHIR_calib/Right/c_energyTOFHIR_calib_%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
             delete latex;
             delete c;
             ...
             latex = new TLatex(0.40,0.85,Form("#splitline{bar %02d - TOFHIR calib}{V_{OV} = %.2f V, th. = %d DAC}",iBar,Vov,int(vth)));
             latex -> SetNDC();
             latex -> SetTextFont(42);
             latex -> SetTextSize(0.04);
             latex -> SetTextColor(kBlack);
             ...
             c -> Print(Form("%s/Scatter_plots/Loop3_scatter_energy_TOFHIR_calib_L_vs_R/c_energy_TOFHIR_calib_L_vs_R_%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
             c -> Print(Form("%s/Scatter_plots/Loop3_scatter_energy_TOFHIR_calib_L_vs_R/c_energy_TOFHIR_calib_L_vs_R_%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
             delete c;
             delete latex;
...
// --- Chi2 histograms:
FillAndPlotHistos(Chi2_values_LO_map,"Chi2_LO","#chi^{2}/NDF",50, 0., 10.,0., 50.,plotDir + "/MPV_values/Chi2_histograms");
FillAndPlotHistos(Chi2_values_COMBINED_map,"Chi2_COMBINED","#chi^{2}/NDF",50, 0., 10.,0., 50.,plotDir + "/MPV_values/Chi2_histograms");
FillAndPlotHistos(Chi2_values_RESIDUAL_TOFHIR_map,"Chi2_RESIDUAL_TOFHIR","#chi^{2}/NDF",50, 0., 10.,0., 50.,plotDir + "/MPV_values/Chi2_histograms");
FillAndPlotHistos(Chi2_values_TOFHIR_map,"Chi2_TOFHIR","#chi^{2}/NDF",50, 0., 10.,0., 50.,plotDir + "/MPV_values/Chi2_histograms");

// --- MPV histograms:
FillAndPlotHistos(MPV_RAW_map, "MPV_RAW", "MPV [a.u.]", 100, 0., 1000., 50., 900., plotDir+"/MPV_values/MPV_histograms");
FillAndPlotHistos(MPV_LO_map, "MPV_LO", "MPV [a.u.]", 100, 0., 1000., 50., 900., plotDir+"/MPV_values/MPV_histograms");
FillAndPlotHistos(MPV_RESIDUAL_TOFHIR_map, "MPV_RESIDUALTOFHIR", "MPV [a.u.]", 100, 0., 1000., 50., 900., plotDir+"/MPV_values/MPV_histograms");
FillAndPlotHistos(MPV_TOFHIR_map, "MPV_TOFHIR", "MPV [a.u.]", 100, 0., 1000., 50., 900., plotDir+"/MPV_values/MPV_histograms");
FillAndPlotHistos(MPV_COMBINED_map, "MPV_COMBINED", "MPV [a.u.]", 100, 0., 1000., 50., 900., plotDir+"/MPV_values/MPV_histograms");

std::cout << std::endl;
std::cout << "[HISTOGRAMS MVP AND Chi2 VALUES] --> done!" << std::endl;
std::cout << std::endl;
...
BuildGraphs(MPV_RAW_map,g_MPV_RAW_L,g_MPV_RAW_R);
PlotGraphs(g_MPV_RAW_L,g_MPV_RAW_R,"MPV_RAW","MPV raw [a.u.]",plotDir + "/MPV_values/MPV_RAW",200, 500);


//-------------------------------------------------------
// MPV values histograms energy with LO equalization
//-------------------------------------------------------
BuildGraphs(MPV_LO_map,g_MPV_LO_L,g_MPV_LO_R);
PlotGraphs(g_MPV_LO_L,g_MPV_LO_R,"MPV_LO","MPV LO [a.u.]",plotDir + "/MPV_values/MPV_LO_calib",200, 500);


//------------------------------
// MPV COMBINED caliabration
//------------------------------
BuildGraphs(MPV_COMBINED_map,g_MPV_COMBINED_L,g_MPV_COMBINED_R);
PlotGraphs(g_MPV_COMBINED_L,g_MPV_COMBINED_R,"MPV_COMBINED","MPV Combined [a.u.]",plotDir + "/MPV_values/MPV_COMBINED_calib",200, 500);


//----------------------------
//   MPV RESIDUAL TOFHIR calibration
//---------------------------- 
BuildGraphs(MPV_RESIDUAL_TOFHIR_map,g_MPV_RESIDUAL_TOFHIR_L,g_MPV_RESIDUAL_TOFHIR_R);
PlotGraphs(g_MPV_RESIDUAL_TOFHIR_L,g_MPV_RESIDUAL_TOFHIR_R,"MPV_RESIDUAL_TOFHIR","MPV Residual TOFHIR [a.u.]",plotDir + "/MPV_values/MPV_RESIDUAL_TOFHIR_calib",200, 500);


//----------------------------
//   MPV TOFHIR calibration
//---------------------------- 
BuildGraphs(MPV_TOFHIR_map,g_MPV_TOFHIR_L,g_MPV_TOFHIR_R);
PlotGraphs(g_MPV_TOFHIR_L,g_MPV_TOFHIR_R,"MPV_TOFHIR","MPV TOFHIR [a.u.]",plotDir + "/MPV_values/MPV_TOFHIR_calib",200, 500);


std::cout << std::endl;
std::cout << "[PLOTS MPV VALUES vs BAR] ---> done!" << std::endl;
std::cout << std::endl;
...
// -------------------------------------------------------
//  SUMMARY MPV plots (RAW, LO, RESIDUAL_TOFHIR, COMBINED)
// -------------------------------------------------------
...
for (auto &it : g_MPV_RAW_L) // loop su tutte le chiavi (vov, vth)
{
    ...
    c_L->Print(Form("%s/MPV_values/MPV_summary/MPV_summary_L_Vov%.2f_th%02d.pdf", plotDir.c_str(), vov, vth));
    c_L->Print(Form("%s/MPV_values/MPV_summary/MPV_summary_L_Vov%.2f_th%02d.png", plotDir.c_str(), vov, vth));
    ...
    c_R->Print(Form("%s/MPV_values/MPV_summary/MPV_summary_R_Vov%.2f_th%02d.pdf", plotDir.c_str(), vov, vth));
    c_R->Print(Form("%s/MPV_values/MPV_summary/MPV_summary_R_Vov%.2f_th%02d.png", plotDir.c_str(), vov, vth));

    delete c_R;
}
std::cout << std::endl;
std::cout << "[SUMMARY PLOTS MPV VALUES] ---> done!" << std::endl;
std::cout << std::endl;



////////////////////////////////
//   Plot  MPV RATIO vs bar   //
////////////////////////////////

//-------------------------------------------------------------------
//  MPV RATIO values without any calibration (raw):
//-------------------------------------------------------------------
BuildMPVRatioGraphs(g_MPV_RAW_L, g_MPV_RAW_R, gMPV_Ratio_RAW);
PlotMPVRatioGraphs(gMPV_Ratio_RAW, "RAW", plotDir + "/MPV_Ratio/MPV_ratio_RAW");

//-------------------------------------------------------------------
//  MPV RATIO values with LO calibration:
//-------------------------------------------------------------------
BuildMPVRatioGraphs(g_MPV_LO_L, g_MPV_LO_R, gMPV_LO_Ratio);
PlotMPVRatioGraphs(gMPV_LO_Ratio, "LO calibration", plotDir + "/MPV_Ratio/MPV_Ratio_LO");

//-------------------------------------------------------------------
//  MPV RATIO values with RESIDUAL TOFHIR calibration:
//-------------------------------------------------------------------
BuildMPVRatioGraphs(g_MPV_RESIDUAL_TOFHIR_L, g_MPV_RESIDUAL_TOFHIR_R, gMPV_RESIDUAL_TOFHIR_Ratio);
PlotMPVRatioGraphs(gMPV_RESIDUAL_TOFHIR_Ratio, "RESIDUAL TOFHIR calibration", plotDir + "/MPV_Ratio/MPV_Ratio_RESIDUAL_TOFHIR");

//-------------------------------------------------------------------
//  MPV RATIO values with TOFHIR calibration:
//-------------------------------------------------------------------
BuildMPVRatioGraphs(g_MPV_TOFHIR_L, g_MPV_TOFHIR_R, gMPV_TOFHIR_Ratio);
PlotMPVRatioGraphs(gMPV_TOFHIR_Ratio, "TOFHIR calibration", plotDir + "/MPV_Ratio/MPV_Ratio_TOFHIR");

//-------------------------------------------------------------------
//  MPV RATIO values with COMBINED calibration:
//-------------------------------------------------------------------
BuildMPVRatioGraphs(g_MPV_COMBINED_L, g_MPV_COMBINED_R, gMPV_COMBINED_Ratio);
PlotMPVRatioGraphs(gMPV_COMBINED_Ratio, "COMBINED calibration", plotDir + "/MPV_Ratio/MPV_Ratio_COMBINED");

std::cout << std::endl;
std::cout << "[PLOTS MPV VALUES RATIO] ---> done!" << std::endl;
std::cout << std::endl;
